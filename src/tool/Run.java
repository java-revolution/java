...
public class Run extends TimerTask {
	...
	public void run() {
		...
		for(int i = 0;i < Csv.csvData.size();i++){
			...
			System.out.println("ファイルサイズ"+i+"番目："+BEFORE_FILE_SIZE[i]+"byte");
			System.out.println("タスク実行：" + new Date());
		
		    if(BEFORE_FILE_SIZE[i] == 0){
		    	// TaskTray.ICON.displayMessage("メッセージ", "ファイルの大きさが0です", TrayIcon.MessageType.WARNING);
		    	AFTER_FILE_SIZE[i]=BEFORE_FILE_SIZE[i];
		    }
		            	
		    if(AFTER_FILE_SIZE[i] == BEFORE_FILE_SIZE[i]){
		    	// TaskTray.ICON.displayMessage("メッセージ", "ファイルの大きさが同じです", TrayIcon.MessageType.WARNING);
		    	AFTER_FILE_SIZE[i]=BEFORE_FILE_SIZE[i];
		    }
	            	
		    else if (AFTER_FILE_SIZE[i] != BEFORE_FILE_SIZE[i]) {
		    	TaskTray.ICON.displayMessage("メッセージ", Csv.csvData.get(i)+"が更新されました", TrayIcon.MessageType.INFO);
		    	AFTER_FILE_SIZE[i]=BEFORE_FILE_SIZE[i];
		    }
		    else{
